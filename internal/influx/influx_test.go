...
func queryFnTestHelper(t *testing.T, dir string) func(q client.Query) (*client.Response, error) {
	...
	return func(q client.Query) (*client.Response, error) {
		stmt, err := influxql.NewParser(strings.NewReader(q.Command)).ParseStatement()
		if err != nil {
			t.Fatal(err)
		}
		...
		default:
			...
			m := selectStmt.Sources.Measurements()
			if len(m) == 0 {
				t.Fatal("No measurement found in query")
			}

			return responseFromFile(t, filepath.Join(dir, m[0].Name+".json"))
		}
